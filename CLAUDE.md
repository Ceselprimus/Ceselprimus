...
## 현재 상태 (2026-07-06 기준)
- 인사이트 15편. 최신: 10탄 `knowledge-center-chp-strawberry-farm`(지식산업센터 모델, 한글 전용), 11탄 `alphafarm-low-power-dehumidification`(저전력 저온제습, 국·영문).
- 시리즈 1~11 인포그래픽 반영. 홈 '사업 모델 제안' 6종(국문)/5종(영문). GA4·사이트맵·게시판·영문 사이트 정상.
- '딸기수직농장 = 홈페이지' SEO 방침 적용 완료: 홈 H1/title 키워드 앞배치, 랜딩 4개는 홈 보조 구조(키워드 앵커로 홈 링크), 인사이트 13편+ 하단 홈 키워드 링크.
...
- 대기 중: ① 지식산업센터 모델 글로벌 버전 → 영문 홈 Business Model Proposal 추가(대표 7/6 지시, 파일은 영업DB/홈페이지에 있음 — Drive 공유 또는 docs/ 전달 필요) ② SNS 9~11탄 순차 게시(대표 "순서대로 업로드" 지시 대기).

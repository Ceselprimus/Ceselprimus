...
export default function SiteHeader({ inquiryHref }: { inquiryHref: string }) {
    ...
    <>
      ...
      >
        <div className="mx-auto flex h-[68px] w-[min(1140px,92vw)] items-center justify-between md:h-[78px]">
          <a href="#top" aria-label="쎄슬프라이머스 홈" onClick={() => setOpen(false)}>
            <Image
          ...
          <nav className="hidden items-center gap-9 md:flex" aria-label="주요 메뉴">
            ...
            >
              사업개발 문의 <ArrowUpRight className="h-4 w-4" />
            </a>
          </nav>
      ...
      {open ? (
        <div className="fixed inset-0 z-40 flex flex-col bg-ink px-8 pb-12 pt-28 text-white md:hidden">
          ...
          <div className="mt-auto space-y-4">
            ...
            >
              사업개발 문의 <ArrowUpRight className="h-5 w-5" />
            </a>
            <p className="text-center text-[0.95rem] text-white/56">
              강호수 프로 010-3341-6036 · 이관호 대표 +82 10-3761-7953
            </p>
          </div>
        </div>
      ) : null}
